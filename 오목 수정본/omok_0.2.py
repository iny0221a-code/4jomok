...
# os, sys: 이미지를 불러오는 함수를 작성하기 위한 라이브러리

# 메모: 
# 마지막으로 둔 돌의 이미지를 다르게 하는 기능 필요함

# 수정됨:
# --- 이미지 로드 ---
# 1. 이미지를 불러오는 함수 추가
# 2. 마지막으로 둔 돌을 표현하는 이미지 2종 추가

# --- draw_board 함수 ---
# 1. 배경 이미지를 구현했기 때문에 격자 함수를 주석처리 함
...
# 현재 파이썬 파일이 존재하는 디렉토리의 절대 경로를 main_dir에 저장
main_dir = os.path.split(os.path.abspath(__file__))[0] # __file__: 현재 스크립트 파일의 경로를 가져옴
                                                                                       # os.path.abspath(): 이 경로를 절대 경로로 만듦
                                                                                       # os.path.split(): 이 절대 경로를 디렉토리와 파일 이름으로 분리
                                                                                       # [0]: 분리된 결과 중 디렉토리 부분만 선택

# 이미지를 불러오는 함수
def load_image(name) : # name: 이미지 이름
    path = os.path.join(main_dir, "image", name) # os.path.join(): 경로 결합 함수
                                                                                # "image": 이미지가 들어있는 파일 이름
                                                                                
    return pygame.image.load(path).convert_alpha() # convert_alpha(): png 이미지의 투명도를 살리기 위한 함수
