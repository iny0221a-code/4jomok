...
# 게임 중 메인 메뉴 버튼
main_button_in_game_rect = btn_main_img.get_rect(center=(ui_center_x, text_board_rect.bottom + 140))
quit_button_game_rect = btn_quit_game_img.get_rect(center=(ui_center_x, text_board_rect.bottom + 220))
...
def trigger_chaos_event():
    ...
    if event_type == 1: # 돌 증발 (범위: 마지막 돌 기준 1~4칸)
        if last_r is None:
    ...
    elif event_type == 3: # 돌 솟아남 (범위: 마지막 돌 기준 1~4칸)
        if last_r is None:
...
def reset_game():
    ...
    global is_double_turn_active, double_turn_count, is_color_unified_active, color_unified_turn_count
...
def draw_ui_panel():
    ...
    if game_state == "PLAY" and is_time_attack_active:
        ...
        time_rect = time_surf.get_rect(center=(ui_center_x, text_board_rect.top + 50))
        screen.blit(time_surf, time_rect)

    # 4. 난장판 모드 텍스트 (배경 및 타이머 위에 그림)
    if current_mode == "CHAOS":
        # 이벤트 메시지 표시
        if event_message_timer > 0:
            msg_y_offset = 110 if is_time_attack_active else 70
            msg_surf = font_small.render(last_event_message, True, WOOD_TEXT_COLOR)
            msg_rect = msg_surf.get_rect(center=(text_board_rect.centerx, text_board_rect.top + msg_y_offset))
